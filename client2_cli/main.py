...
def handle_request(client_socket):
    with client_socket:
        ...
        if request['type'] == 'GET_FILE_STATUS':
            ...
            if not info_hash in data or not data[info_hash]:
...
def fetch_file(server_url, info_hash):
    ...
    if response and response.status_code == 200 and response.json() and response.json()['data']:
        ...
        for item in response.json()['data']:
            peers_info = ', '.join([f"{peer['address']}:{peer['port']}" for peer in item['peers']])
            if 'user' in item and item['user']:
                if 'firstName' in item['user'] and 'lastName' in item['user']:
                    user_full_name = f"{item['user']['firstName']} {item['user']['lastName']}"
                else:
                    user_full_name = None
            else:
                user_full_name = None
